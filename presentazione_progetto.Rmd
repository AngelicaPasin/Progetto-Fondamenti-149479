...
I primi 5 records della tabella contenente tutte le informazioni
```{r echo=FALSE}
...
ggplot(season, aes(x=Year, y=total, colour=total, group=Sex))+
  ...
  ggtitle("Numero di atleti maschi e femmine negli anni", subtitle = "Dal 1896 al 2016") 
...
ggplot(men, aes(Name, total, color=Name, fill=Name)) +
  ...
  ggtitle("I migliori vincitori di medaglie", subtitle = "Olimpiadi estive dal 1896 al 2016") 
...
ggplot(men_w, aes(Name, total, color=Name, fill=Name)) +
  ...
  theme(legend.position = "none",
        axis.text = element_text(size = 8, face = "bold"),
        plot.title = element_text(size = 16, face = "bold")) + 
  ggtitle("I migliori vincitori di medaglie", subtitle = "Olimpiadi invernali dal 1924 al 2016") 
...
ggplot(women, aes(Name, total, color=Name, fill=Name)) +
  ...
  theme(legend.position = "none",
        axis.text = element_text(size = 8, face = "bold"),
        plot.title = element_text(size = 16, face = "bold")) + 
  ggtitle("Le migliori vincitrici di medaglie", subtitle ="Olimpiadi estive dal 1896 al 2016") 
...
ggplot(women_w, aes(Name, total, color=Name, fill=Name)) +
  ...
  theme(legend.position = "none",
        axis.text = element_text(size = 8, face = "bold"),
        plot.title = element_text(size = 16, face = "bold")) + 
  ggtitle("Le migliori vincitrici di medaglie", subtitle ="Olimpiadi invernali dal 1924 al 2016") 
...
age_min <- data_events %>%
  filter(!is.na(Medal), Season=="Summer") %>% right_join(age_mi, by=c("Sex","Sport","Age"))
...
ggplot(medals, aes(region, total, color=region, fill=region)) +
  ...
  theme(legend.position = "none",
        axis.text = element_text(size = 8, face = "bold"),
        plot.title = element_text(size = 16, face = "bold")) + 
  ggtitle("I migliori vincitori di medaglie per Paese", subtitle= "Olimpiadi estive dal 1896 al 2016") 
...
# Il numero totale di medaglie vinte da ciascun Team
medalCounts <- data_events %>% filter(!is.na(Medal)) %>% 
  group_by(NOC, Medal, Event, Games) %>%
  summarize(isMedal=1)
medalCounts <- medalCounts %>% 
  group_by(NOC, Medal) %>%
  summarize(Count= sum(isMedal))
medalCounts <- left_join(medalCounts,data_team, by= "NOC")
medalCounts <- medalCounts %>% 
  mutate (Team = region)
medalCounts <- medalCounts %>% select( Medal, Team, Count)

# Ordino i Team per numero di medaglie vinte
...
medals_w$notes = NULL
medals_w =medals_w%>%group_by(region)%>%summarize(total=n())%>%arrange(desc(total))%>%head(n=30)

medals_w$region <- factor(medals_w$region,levels = medals_w$region[order(medals_w$total)])
...
ggplot(medals_w,aes(region,total,color=region,fill=region)) +
  ...
  theme(legend.position = "none",
        axis.text = element_text(size = 8, face = "bold"),
        plot.title = element_text(size = 16, face = "bold")) + 
  ggtitle("I migliori vincitori di medaglie per Paese", subtitle= "Olimpiadi invernali dal 1924 al 2016") 
...
L'Italia, nella storia delle Olimpiadi esive, ha ottenuto 492 ori, 467 argenti e 425 bronzi.

```{r echo=FALSE}
medals_it = data_events%>%filter(!is.na(Medal),Team=='Italy',Season=='Summer') %>%
...
ggplot(medals_it, aes(Medaglia, Totale, fill=Medaglia)) +
  ...
  theme(legend.position = "none",
        axis.text = element_text(size = 8, face = "bold"),
        plot.title = element_text(size = 16, face = "bold")) + 
  ggtitle("Medaglie vinte dall'Italia", subtitle= "Olimpiadi estive dal 1896 al 2016")
```


Alle Olimpiadi invernali, invece, l'Italia ha ottenuto 43 ori, 41 argenti e 59 bronzi.

```{r echo=FALSE}
medals_itw = data_events%>%filter(!is.na(Medal), Team=='Italy', Season=='Winter') %>%
  group_by(Medaglia=Medal) %>%
  summarise(Totale=n()) %>%
  arrange(-Totale)
...
ggplot(medals_itw,aes(Medaglia, Totale, fill=Medaglia)) +
  ...
  theme(legend.position = "none",
        axis.text = element_text(size = 8, face = "bold"),
        plot.title = element_text(size = 16, face = "bold")) + 
  ggtitle("Medaglie vinte dall'Italia", subtitle= "Olimpiadi invernali dal 1924 al 2016")
```
...
sport_m <- data_events %>%
  filter(!is.na(Age),!is.na(Height),!is.na(Weight),!is.na(Medal),Season=='Summer',Year==2016)
...
ggplot(age,aes(x=Age))+
  ...
  theme(legend.position = "none",
        axis.text = element_text(size = 8, face = "bold"),
        plot.title = element_text(size = 16, face = "bold")) + 
  ggtitle("Distribuzione per età degli atleti", subtitle = "Olimpiadi estive dal 1896 al 2016") 
...
age_s <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Swimming")%>%group_by(Year)%>%summarize(avg=round(mean(Age,na.rm = TRUE),1)) #arrotondo il valore a 1 cifra decimale
age_a <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Athletics")%>%group_by(Year)%>%summarize(avg=round(mean(Age,na.rm = TRUE),1))

age_g <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Gymnastics")%>%group_by(Year)%>%summarize(avg=round(mean(Age,na.rm = TRUE),1))

age_y = data_events%>%filter(Season=='Summer')%>%group_by(Year)%>%summarize(Y=unique(Year))
...
weight_spw <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Weightlifting")%>%group_by(Year)%>%summarize(avg=round(mean(Weight,na.rm = TRUE),1))
weight_spb <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Boxing")%>%group_by(Year)%>%summarize(avg=round(mean(Weight,na.rm = TRUE),1))

weight_spd <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Diving")%>%group_by(Year)%>%summarize(avg=round(mean(Weight,na.rm = TRUE),1))

weight_sph <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Hockey")%>%group_by(Year)%>%summarize(avg=round(mean(Weight,na.rm = TRUE),1))

weight_swr <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Wrestling")%>%group_by(Year)%>%summarize(avg=round(mean(Weight,na.rm = TRUE),1))
...
weight_mt <- data_events%>%filter(!is.na(Medal),Season=='Summer')%>%group_by(Sex,Sport)%>%summarize(Weight=max(Weight,na.rm = TRUE))%>%arrange(desc(Weight))

weight_mt<- weight_mt[-c(76:87),]

weight_max <- data_events%>%filter(!is.na(Medal),Season=="Summer")%>%right_join(weight_mt,by=c("Sex","Sport","Weight"))
...
height_spw <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Volleyball")%>%group_by(Year)%>%summarize(avg=round(mean(Height,na.rm = TRUE),1))
height_spb <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Basketball")%>%group_by(Year)%>%summarize(avg=round(mean(Height,na.rm = TRUE),1))

height_spd <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Football")%>%group_by(Year)%>%summarize(avg=round(mean(Height,na.rm = TRUE),1))

height_sph <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Rowing")%>%group_by(Year)%>%summarize(avg=round(mean(Height,na.rm = TRUE),1))

height_swr <- data_events%>%filter(Sex=="M",Season=='Summer',Sport=="Athletics")%>%group_by(Year)%>%summarize(avg=round(mean(Height,na.rm = TRUE),1))
...
height_ht <- data_events%>%filter(!is.na(Medal),Season=='Summer')%>%group_by(Sex,Sport)%>%summarize(Height=round(max(Height,na.rm = TRUE),0))%>%arrange(desc(Height))
height_ht <- height_ht[-c(78:87),]

height_max <- data_events%>%filter(!is.na(Medal),Season=="Summer")%>%right_join(height_ht,by=c("Sex","Sport","Height"))
...
Weight_ht<- data_events%>%filter(!is.na(Weight), !is.na(Height), !is.na(Medal), Season=='Summer', Year==2016)
